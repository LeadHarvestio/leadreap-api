...
// ─────────────────────────────────────────────────────────────
export function createJob({ niche, location, limit = 20, offset = 0, scrapeEmails = true, forceRefresh = false }) {
...
async function processQueue() {
  ...
  console.log(`[Queue] Starting job ${jobId}: "${job.niche}" in "${job.location}" (limit: ${job.limit}, offset: ${job.offset})`);
  try {
